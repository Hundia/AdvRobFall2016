...
public class VoiceControl extends BluetoothActivity
{
	...
	// Available commands
	private static final String[] commands = {"formation 1","formation 2"
		, "formation one"
		,"formation two"
		,"move one right"
		,"move 1 right"
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		if(requestCode == BluetoothRemoteControlApp.MSG_1 && resultCode == RESULT_OK)
		{
			...
			tvData.append("Command: " + matches.get(0) + "\n");
			for(String command : commands)
			{
				...
				if(matches.contains(command))
				{
					...
					if(command == "move up")
					{
						write("moveby " + 0 + " " + 0 + " " + 300);
					}
					if(command.contains("move down"))
					{
						write("moveby " + 0 + " " + 0 + " -" + 300);
					}
					//Form Formation
